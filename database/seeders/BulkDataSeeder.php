...
{
    /**
     * Seed N records per logical table group (see run body). N = config ship.bulk_seed_count (env BULK_SEED_COUNT).
     */
    public function run(): void
    {
        $count = (int) config('ship.bulk_seed_count', 100);
        $this->command->info("Starting bulk data seeding ({$count} rows per group)...");
        ...
        $this->command->info("✓ Role-Permissions seeded: {$count}");
        ...
        $this->command->info("✓ User-Roles seeded: {$count}");
