...
1. **Đăng nhập:** `POST /api/v1/auth/login` với JSON `{ "email", "password" }`; response `data.user` và `data.token` (Bearer Sanctum). Chi tiết: [FRONTEND_API_ENDPOINTS.md](./FRONTEND_API_ENDPOINTS.md).
